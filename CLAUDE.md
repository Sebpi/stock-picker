...
- **Refresh token rotation is single-use and atomic.** Each `POST /api/auth/refresh` revokes the old token and issues a new one in a single transaction (conditional `UPDATE ... WHERE revoked = 0` — the loser of a race gets 401, token-theft detection). The frontend doesn't yet handle silent refresh, so 15-minute access tokens will cause visible logouts without a refresh implementation.
