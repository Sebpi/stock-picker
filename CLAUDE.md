...
- **`short_interest`** (`backend/agents/short_interest.py`) reads FINRA short interest via yfinance (`shortPercentOfFloat`, `shortRatio`, `sharesShort`, `sharesShortPriorMonth`). Scores on short % of float (low = bullish) and MoM change direction (falling = covering = bullish). Squeeze risk bonus when short % ≥ 10% + days-to-cover ≥ 8 + MoM falling. Weighted 0.07 at 3m, 0.05 at 6m, 0.03 at 12m. Scores on put/call ratio (PCR), IV skew (near-ATM put IV minus call IV), and volume direction. PCR < 0.50 → 85 (heavy call bias); PCR > 1.55 → 26 (heavy put buying). Weighted 0.09 at 3m (strongest near-term signal), 0.04 at 6m, 0.02 at 12m. LOW_COVERAGE emitted for stocks with no options or < 100 total chain volume.
