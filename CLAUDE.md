...
- **`capital_allocation`** (`backend/agents/capital_allocation.py`) scores ROE (compounding quality), FCF conversion (FCF/NI > 1.5 → +7), goodwill growth (rising intangibles → acquisition risk penalty), and asset turnover trend. Weighted 0.03 at 3m, 0.05 at 6m, 0.08 at 12m (most impactful long-term).
