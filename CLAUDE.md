...
- **Minimum 3 agents** must return successfully or the thesis is rejected (`MIN_AGENTS_REQUIRED`). Alert snapshot requires **≥5 of 10 agents** net-positive (`alert_min_positive_agents`, now 5-of-10 = 50%). Score → 12-month return uses linear interpolation over the `SCORE_TO_12M_RETURN` anchor table.
