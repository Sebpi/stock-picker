...
- **Fifteen agents** (`credit_risk`, `earnings_quality`, `earnings_surprise`, `fundamentals`, `growth_revisions`, `industry_competition`, `insider_activity`, `institutional_flow`, `macro_liquidity`, `options_flow`, `portfolio_risk`, `sentiment_news`, `short_interest`, `technical_risk`, `valuation`, plus the `orchestrator`). All agents emit `AgentSignal` (defined in `schemas.py`); the orchestrator aggregates them into an `InvestmentThesis` with bull/base/bear `HorizonForecast`s and a Claude-generated narrative.
