...
- **Eleven agents** (`fundamentals`, `growth_revisions`, `industry_competition`, `insider_activity`, `macro_liquidity`, `options_flow`, `portfolio_risk`, `sentiment_news`, `technical_risk`, `valuation`, plus the `orchestrator`). All agents emit `AgentSignal` (defined in `schemas.py`); the orchestrator aggregates them into an `InvestmentThesis` with bull/base/bear `HorizonForecast`s and a Claude-generated narrative.
...
- **`options_flow`** (`backend/agents/options_flow.py`) reads the live yfinance options chain for the nearest 1–3 expiries. Scores on put/call ratio (PCR), IV skew (near-ATM put IV minus call IV), and volume direction. PCR < 0.50 → 85 (heavy call bias); PCR > 1.55 → 26 (heavy put buying). Weighted 0.09 at 3m (strongest near-term signal), 0.04 at 6m, 0.02 at 12m. LOW_COVERAGE emitted for stocks with no options or < 100 total chain volume.
